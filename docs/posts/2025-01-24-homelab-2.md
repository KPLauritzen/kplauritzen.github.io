...
So far it's been a fun experiment to see if I can avoid catastrophically destroying the cluster, now that there is not a full team of experts hired to keep it going (Which is the situation at my job). 

Today I managed to get [FreshRSS](https://freshrss.org/) and [Atuin Sync](https://github.com/ellie/atuin) running.

I've been using [Cursor](https://www.cursor.com/) as a guide generating the yaml files and asking questions about how Kubernetes works. I use Kubernetes a fair bit at work, but I am far from an expert.
